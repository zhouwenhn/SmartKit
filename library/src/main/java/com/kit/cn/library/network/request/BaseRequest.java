...
import com.kit.cn.library.network.cache.CacheManager;
import com.kit.cn.library.network.cache.CacheMode;
import com.kit.cn.library.network.callback.AbsCallback;
...
import com.kit.cn.library.network.utils.HeaderParser;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        ...
        OkHttpTask okHttpUtils = OkHttpTask.getInstance();
    ...
    protected RequestBody wrapRequestBody(RequestBody requestBody) {
        ...
        progressRequestBody.setListener(new ProgressRequestBody.Listener() {
            ...
            public void onRequestProgress(final long bytesWritten, final long contentLength, final long networkSpeed) {
                OkHttpTask.getInstance().getDelivery().post(new Runnable() {
                    @Override
                    public void run() {
    ...
    protected Call generateCall(Request request) {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null && userCookies.size() == 0) {
            return OkHttpTask.getInstance().getOkHttpClient().newCall(request);
        } else {
            okhttp3.OkHttpClient.Builder newClientBuilder = OkHttpTask.getInstance().getOkHttpClient().newBuilder();
            ...
            if (userCookies.size() > 0) OkHttpTask.getInstance().getCookieJar().addCookies(userCookies);
