...
import com.kit.cn.library.network.cookie.store.PersistentCookieStore;
...
//        OkHttpTask okHttpClient = new OkHttpTask.Builder()
...
//                .build();
//        OkHttp.initClient(okHttpClient);
        ...
        //必须调用初始化
        OkHttpTask.init(this);

        //以下设置的所有参数是全局参数,同样的参数可以在请求的时候再设置一遍,那么对于该请求来讲,请求中的参数会覆盖全局参数
        //好处是全局参数统一,特定请求可以特别定制参数
        try {
            //以下都不是必须的，根据需要自行选择,一般来说只需要 debug,缓存相关,cookie相关的 就可以了
            OkHttpTask.getInstance()

                    //打开该调试开关,控制台会使用 红色error 级别打印log,并不是错误,是为了显眼,不需要就不要加入该行
                    .debug("OkHttpTask")

                    //如果使用默认的 60秒,以下三行也不需要传
                    .setConnectTimeout(OkHttpTask.DEFAULT_MILLISECONDS)  //全局的连接超时时间
                    .setReadTimeOut(OkHttpTask.DEFAULT_MILLISECONDS)     //全局的读取超时时间
                    .setWriteTimeOut(OkHttpTask.DEFAULT_MILLISECONDS)    //全局的写入超时时间
